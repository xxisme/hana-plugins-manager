...
// 防超大 zip 整体读入内存造成 OOM（下载源已有 50MB 限制；本地 zip/tool 导入无前置限制，这里兜底）
...
export function extractZip(zipPath, destDir) {
  ...
  if (size > MAX_ZIP_BYTES) {
    throw new Error(`zip 体积超过上限 ${Math.round(MAX_ZIP_BYTES / 1024 / 1024)}MB（实际 ${(size / 1024 / 1024).toFixed(1)}MB），已拒绝`);
  }
