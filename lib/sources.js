 ...
 *
 * @param {object} [opts]
 * @param {boolean} [opts.overwrite=false] 默认 false：已有 source 且与新 url 不同时拒绝覆盖，
 *   返回 { saved:false, reason:'already-set', existing }，保留用户手动设置的关联。
 *   手动关联路由传 true 强制覆盖；自动关联场景（安装时）保持默认即可。
 */
export function setSource(dataDir, pluginId, githubUrl, opts = {}) {
  const overwrite = opts.overwrite === true;
  const sources = readSources(dataDir);
  const url = String(githubUrl || '').trim();
  if (!url) {
    // 清空关联的语义不受 overwrite 控制（卸载等场景需要强制清）
  ...
  // 默认 overwrite=false：已有 source 且与新 url 不同则拒绝覆盖,保留已有设置
  if (!overwrite) {
    const existing = sources[pluginId];
    if (existing && existing.githubUrl && existing.githubUrl !== parsed.url) {
      return { saved: false, reason: 'already-set', existing };
    }
  }
