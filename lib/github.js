...
const REPO_INFO_CACHE = new Map(); // key -> { at, data }
const REPO_INFO_TTL = 5 * 60 * 1000; // 5min 失效，避免默认分支改动后长期读旧数据
const REPO_INFO_INFLIGHT = new Map(); // key -> Promise（并发去重，防重复请求浪费 GitHub 配额）
async function getRepoInfoCached(owner, repoName, dataDir) {
  const key = `${owner}/${repoName}`;
  const hit = REPO_INFO_CACHE.get(key);
  if (hit && Date.now() - hit.at < REPO_INFO_TTL) return hit.data;
  // 同一仓库的并发请求共享同一个 in-flight Promise，避免各跑一次 fetch
  if (REPO_INFO_INFLIGHT.has(key)) return REPO_INFO_INFLIGHT.get(key);
  const p = getRepoInfo(`${owner}/${repoName}`, dataDir)
    .then((r) => {
      REPO_INFO_INFLIGHT.delete(key);
      // 只缓存成功结果：限流(403)/网络失败不缓存，否则失败状态会污染后续所有检测
      if (r.ok) REPO_INFO_CACHE.set(key, { at: Date.now(), data: r });
      return r;
    })
    .catch((e) => {
      REPO_INFO_INFLIGHT.delete(key);
...
async function readVersionFrom(owner, repoName, branch, dir, dataDir, expectId) {
  ...
  if (m.ok) {
    try {
      ...
      // 合集仓库防串版：传了 expectId 时，manifest.id 或目录名必须匹配本地插件，
      // 否则跳过——避免把仓库里其他插件的版本（如改名后的 hana-downloader）
      // 误当成目标插件 download-progress 的版本
      if (j && j.version && (!expectId || String(j.id || '') === expectId || dirBaseName(dir) === expectId)) {
        return { ok: true, version: j.version, from: 'manifest' };
      }
    } catch { /* ignore */ }
  }
  ...
  if (p.ok) {
    try {
      ...
      // package.json 无 id 字段，仅靠目录名匹配（有 id 的都在 manifest 分支处理）
      if (j && j.version && (!expectId || dirBaseName(dir) === expectId)) {
        return { ok: true, version: j.version, from: 'package.json' };
      }
    } catch { /* ignore */ }
  }
 ...
 */
async function findNestedVersion(owner, repoName, branch, dataDir, expectId) {
  const token = readGithubToken(dataDir);
  ...
  for (const cand of candidates) {
    tried += 1;
    const v0 = await readVersionFrom(owner, repoName, branch, cand, dataDir, expectId);
    if (v0.ok) return v0;
    ...
    for (const sd of subDirs.slice(0, 5)) {
      const v1 = await readVersionFrom(owner, repoName, branch, `${cand}/${sd.name}`, dataDir, expectId);
      if (v1.ok) return v1;
    }
  }
  return {
    ok: false, status: 'no-version', noVersion: true,
    error: expectId
      ? `仓库中未找到与插件「${expectId}」匹配的目录（插件可能已改名或移除，请核对 GitHub 仓库）`
      : `已尝试 ${tried} 个嵌套目录，均未找到 version`,
  };
}
 ...
 */
export async function getRemoteVersion(owner, repoName, branch, dataDir, expectId) {
  ...
  // release 先查一次（同仓库跨分支共享）。关键语义：**有 release 的单插件仓库以 release 为权威**——
  // 只有 release 才有可下载的构建产物；源码 manifest 可能超前于 release（作者先 bump 源码后发版，
  // 如 dsh-hanako 源码 1.0.0-beta 而 release 仅 v0.23.x），此时报源码版本会导致
  // 「检测显示可更新 X、下载却装成 Y」的死循环。检测=可下载版本，两者永远同源。
  const rel = await getLatestRelease(owner, repoName, dataDir);
  const relTagNum = rel.ok && rel.tag ? String(rel.tag).replace(/^v/i, '') : null;
  const relUsable = !!relTagNum && /^\d/.test(relTagNum);

  // 候选分支顺序：默认分支 > 用户配置分支 > main > master
  const branches = Array.from(new Set([info.defaultBranch, branch, 'main', 'master'].filter(Boolean)));
  for (const br of branches) {
    // 2) 根目录 manifest 存在 = 单插件仓库：release 优先，无 release 用源码 manifest 版本
    const rootM = await getRemoteFile(owner, repoName, br, 'manifest.json', dataDir);
    if (rootM.ok) {
      let m = null;
      try { m = JSON.parse(rootM.content); } catch { /* ignore */ }
      if (m && m.version) {
        // 根 manifest.id 与本地插件不符：关联可能错位/插件改 id，跳过该分支（交给 no-version 提示）
        if (expectId && m.id && String(m.id) !== expectId) continue;
        if (relUsable) return { ok: true, version: relTagNum, from: 'release' };
        return { ok: true, version: m.version, from: 'manifest' };
      }
    }
    // 2b) 根无 manifest 但有 package.json：构建型单插件项目（manifest 由构建生成，
    //     如 dsh-hanako 仓库重构后根目录只有 package.json + src/）。此时 release 是唯一
    //     可下载交付物，同样以 release 为权威（防源码子目录版本超前 release 造成死循环）
    const rootP = await getRemoteFile(owner, repoName, br, 'package.json', dataDir);
    if (rootP.ok && relUsable) {
      let p = null;
      try { p = JSON.parse(rootP.content); } catch { /* ignore */ }
      if (p && p.version) {
        // expectId 守护：package.json 的 name（去 @scope/ 前缀）须与本插件相关，
        // 防止用户关联错仓库时把别的插件的 release 误当成本插件的版本（误报可更新）
        const pkgName = String(p.name || '').trim().replace(/^@[^/]+\//, '').toLowerCase();
        const id = String(expectId || '').toLowerCase();
        const idMatch = !id || (pkgName && (pkgName === id || pkgName.includes(id) || id.includes(pkgName)));
        if (idMatch) return { ok: true, version: relTagNum, from: 'release' };
      }
    }
    // 3) 根无 manifest/package.json（合集/monorepo）：按 expectId 匹配子目录源码版本
    const nested = await findNestedVersion(owner, repoName, br, dataDir, expectId);
  ...
  // 4) 兜底：理论上是合集无匹配（expectId 场景已报详细错误），或纯根 manifest 缺 version
  if (!expectId && relUsable) return { ok: true, version: relTagNum, from: 'release' };
  return {
    ok: false, status: 'no-version',
    error: expectId
      ? `远端仓库中未找到插件「${expectId}」（可能已改名或移除，请核对 GitHub 仓库）`
      : '远端仓库无 version 字段（manifest/package.json/release tag 均无 version）',
  };
}
