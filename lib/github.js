...
async function readVersionFrom(owner, repoName, branch, dir, dataDir) {
  ...
  const upstreamStatus = (m && m.status) || (p && p.status) || null;
  return { ok: false, noVersion: true, upstreamStatus, error: upstreamStatus === 404 ? '文件不存在' : null };
}

// 常见的"插件位于子目录"命名，匹配后可下探该目录
const NESTED_CANDIDATES = ['plugins', 'plugin', 'packages', 'src', 'apps', 'tools', 'services'];

/**
 * 在嵌套目录中探测 version：兼容 monorepo 单包裹、多插件仓库等非规范形态。
 * 返回 { ok } 表示找到；否则 { ok: false, noVersion: true, upstreamStatus } 让上层决定如何归类。
 */
async function findNestedVersion(owner, repoName, branch, dataDir) {
  ...
  const listDir = async (p) => {
    try {
      ...
      if (!res.ok) return null; // 列表 404 通常意味着仓库不存在
      const j = await res.json();
      return Array.isArray(j) ? j : null;
    } catch { return null; }
  };
  const isUsableDir = (i) => i.type === 'dir' && !['node_modules', '.git', '.github', 'docs', 'dist', 'build'].includes(i.name);

  const root = await listDir('');
  if (!root) return { ok: false, noVersion: true, upstreamStatus: 404, error: '仓库不存在或无权限' };
  const dirs = root.filter(isUsableDir);

  const candidates = [];
  if (dirs.length === 1) candidates.push(dirs[0].name); // repo-main 之类单包裹
  for (const d of NESTED_CANDIDATES) {
    if (dirs.find((x) => x.name === d)) candidates.push(d);
  }
  ...
  for (const cand of candidates) {
    ...
    if (v0.ok) return v0;
    if (v0.upstreamStatus === 404) return v0; // 子目录 404 重要
    if (!bestStatus && v0.upstreamStatus) bestStatus = v0.upstreamStatus;
    const sub = await listDir(cand);
    if (!sub) continue;
    const subDirs = sub.filter(isUsableDir);
    if (subDirs.length === 1) {
      const v1 = await readVersionFrom(owner, repoName, branch, `${cand}/${subDirs[0].name}`, dataDir);
      if (v1.ok) return v1;
      if (v1.upstreamStatus === 404) return v1;
    }
  }
  return { ok: false, noVersion: true, upstreamStatus: bestStatus, error: '嵌套目录无 version' };
}
 ...
 * 失败时细分状态：
 *  - status: 'upstream-404'  仓库/文件不存在、链接错误或私有仓库无权限
 *  - status: 'no-version'    仓库存在但 manifest/package.json 均无 version 字段、也无 release tag
 *  - status: 'check-failed'  网络/限流等其他失败
 */
export async function getRemoteVersion(owner, repoName, branch, dataDir) {
  const root = await readVersionFrom(owner, repoName, branch, '', dataDir);
  if (root.ok) return root;
  const nested = await findNestedVersion(owner, repoName, branch, dataDir);
  if (nested.ok) return nested;
  // 综合状态码
  const upstreamStatus = (nested && nested.upstreamStatus) || (root && root.upstreamStatus) || null;
  if (upstreamStatus === 404) {
    return { ok: false, status: 'upstream-404', error: 'GitHub 仓库或文件返回 404（仓库不存在、链接错误或私有仓库无权限）' };
  }
  // 200 但无 version：尝试 release tag 兜底
