...
export async function getRemoteManifest(owner, repoName, branch, dataDir) {
  const token = readGithubToken(dataDir);
  const enc = encodeURIComponent(`${owner}/${repoName}`);
  try {
    const res = await fetch(`https://raw.githubusercontent.com/${enc}/${encodeURIComponent(branch || 'master')}/manifest.json`, {
      headers: { 'user-agent': 'hana-plugins-manager' },
      signal: AbortSignal.timeout(FETCH_TIMEOUT),
    });
    if (!res.ok) return { ok: false, status: res.status, error: `远端 manifest 获取失败: HTTP ${res.status}` };
    ...
    return { ok: false, error: `远端 manifest 请求失败: ${e.message || e.name}` };
  }
}
...
export async function getRemoteVersion(owner, repoName, branch, dataDir) {
  ...
  const enc = encodeURIComponent(`${owner}/${repoName}`);
  try {
    const res = await fetch(`https://raw.githubusercontent.com/${enc}/${encodeURIComponent(branch || 'master')}/package.json`, {
      headers: { 'user-agent': 'hana-plugins-manager' },
      signal: AbortSignal.timeout(FETCH_TIMEOUT),
    });
    if (res.ok) {
  ...
  return r.ok ? { ok: false, error: '远端未找到 version（manifest/package.json 均缺失）' } : r;
}
