...
export async function prepareUpdate(pluginId, dataDir) {
  ...
  // pluginRoot 必须显式返回：apply 路由用它调宿主 /api/plugins/install，
  // 漏了会导致 path=undefined → 宿主报 "path is required"
  return { ok: true, check, risk, githubUrl: src.githubUrl, zipPath, pluginRoot: check.pluginRoot };
}
