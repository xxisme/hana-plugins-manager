...
export async function checkUpdates(plugins, dataDir) {
  ...
  const results = await Promise.allSettled(
    targets.map(async (p) => {
      ...
      const rv = await getRemoteVersion(owner, repoName, src.branch, dataDir, p.id);
      if (!rv.ok) {
...
export async function prepareUpdate(pluginId, dataDir) {
  ...
  const zipPath = path.join(workRoot, `${safeId}-${Date.now()}-${Math.random().toString(36).slice(2, 8)}.zip`);

  // 下载（release 优先）+ 结构校验；release 包结构不可用时回退源码 zip，与安装链路 github-apply 一致
  let dl = await downloadZip(src, zipPath, dataDir);
  let check = dl.ok ? checkLocalSource(zipPath, workRoot) : null;
  if (dl.ok && dl.source === 'release' && (!check || !check.ok)) {
    // release 包内容/结构异常（套目录、非插件包等）：清理后重新下载源码
    try { if (check) check.cleanup(); } catch { /* ignore */ }
    try { fs.rmSync(zipPath, { force: true }); } catch { /* ignore */ }
    dl = await downloadZip(src, zipPath, dataDir, false);
    check = dl.ok ? checkLocalSource(zipPath, workRoot) : null;
  }
  if (!dl.ok) {
    try { fs.rmSync(zipPath, { force: true }); } catch { /* ignore */ }
  ...
  if (!check || !check.ok) {
    const detail = check ? check.errors.join('；') + (check.warnings && check.warnings.length ? '（' + check.warnings.join('；') + '）' : '') : dl.error || '结构校验失败';
  ...
  return { ok: true, check, risk, githubUrl: src.githubUrl, zipPath, pluginRoot: installRoot || check.pluginRoot, source: dl.source || 'source' };
}
...
function downloadZip(src, zipPath, dataDir, preferRelease = true) {
  ...
  return import('./github.js').then(async (gh) => {
    // preferRelease=false：强制下载源码（release 包结构校验失败后的回退路径）
    if (preferRelease) {
      // 优先 Release 发布包（作者构建产物，含真实入口；源码项目装源码会因 entry 缺失校验失败）。
      // 无可用资产 / 下载失败时回退源码 zip，Release 是软优先不阻塞更新。
      ...
      if (rel.ok && Array.isArray(rel.assets) && rel.assets.length) {
        ...
        if (dlRel.ok) return { ok: true, path: dlRel.path, size: dlRel.size, source: 'release' };
        try { fs.rmSync(zipPath, { force: true }); } catch { /* ignore */ }
      }
    }
    const dl = await gh.downloadRepoZip(owner, repoName, src.branch, zipPath, dataDir);
    return dl.ok ? { ...dl, source: 'source' } : dl;
  });
}
